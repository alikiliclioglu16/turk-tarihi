...
const TEN = "#D9B48F";
const TEN_GOLGE = "#C08F68";
const CUBBE = "#7A6248";       // kahverengi hırka
const CUBBE_IC = "#E2D5B8";    // krem iç kaftan
const KUSAK = "#B8332B";       // kırmızı kuşak
const BORK = "#6B5237";        // kahverengi börk
const AK = "#F2EDE2";          // sakal
const DERI = "#4A3520";
const ALTIN = "#C9A24B";
 ...
 */
export const DedeKorkut3D = forwardRef<THREE.Group, Props>(function DedeKorkut3D(
  {
          ...
          <group ref={basRef} position={[0, 1.55, 0.01]}>
            ...
            {[-1, 1].map((s) => (
              <mesh key={`b${s}`} position={[0.028 * s, -0.03, 0.09]} rotation={[0.18, 0, -0.55 * s]}>
                <capsuleGeometry args={[0.017, 0.05, 5, 12]} />
                <meshStandardMaterial color={AK} roughness={1} />
              </mesh>
            ))}
            {/* sakal — üç katman, göğse doğru genişleyip incelen */}
            <mesh position={[0, -0.115, 0.045]} rotation={[0.14, 0, 0]} castShadow>
              <capsuleGeometry args={[0.082, 0.14, 8, 16]} />
              <meshStandardMaterial color={AK} roughness={1} />
            </mesh>
            <mesh position={[0, -0.225, 0.035]} rotation={[0.1, 0, 0]} castShadow>
              <capsuleGeometry args={[0.065, 0.12, 8, 14]} />
              <meshStandardMaterial color={AK} roughness={1} />
            </mesh>
            ...
            {/* yanak sakalı */}
            {[-1, 1].map((s) => (
              <mesh key={`f${s}`} position={[0.079 * s, -0.045, 0.025]} scale={[0.48, 1.15, 0.85]}>
                <sphereGeometry args={[0.047, 12, 12]} />
                <meshStandardMaterial color={AK} roughness={1} />
              </mesh>
            ))}

            {/* börk — kürk kenarlı yuvarlak başlık */}
            <mesh position={[0, 0.152, -0.004]} scale={[1, 0.85, 1]} castShadow>
              <sphereGeometry args={[0.115, 20, 16, 0, Math.PI * 2, 0, Math.PI * 0.62]} />
              <meshStandardMaterial color={BORK} roughness={0.9} />
            </mesh>
            <mesh position={[0, 0.108, 0]}>
              <cylinderGeometry args={[0.122, 0.126, 0.055, 20]} />
              <meshStandardMaterial color="#8A7154" roughness={1} />
            </mesh>
          </group>
